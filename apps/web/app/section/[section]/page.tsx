...
async function getStories(section: string) {
  ...
  try { const response = await fetch(`${api}/public/articles?section=${encodeURIComponent(section)}`, { next: { revalidate: 30 } }); return response.ok ? response.json() : []; } catch { return []; }
}

export default async function SectionPage({ params }: { params: Promise<{ section: string }> }) {
  const { section } = await params; const key = section.toLowerCase(); const name = sectionNames[key] ?? section; const stories = await getStories(key);
  return <><ReaderHeader /><main className="section-page"><p className="eyebrow">SECTION</p><h1>{name}</h1><p className="section-intro">The latest reporting, explainers, and analysis from {name.toLowerCase()}.</p>
