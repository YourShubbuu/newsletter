...
export function ArticleActions({ slug }: { slug: string }) {
  ...
  useEffect(() => { fetch(`${apiBase()}/public/saved/${encodeURIComponent(slug)}`, { credentials: 'include' }).then(r => r.ok ? r.json() : null).then(d => { if (d) { setSaved(Boolean(d.saved)); setAccountSaved(true); } }).catch(() => { try { setSaved(window.localStorage.getItem(storageKey) === '1'); } catch {} }); }, [slug, storageKey]);
  async function toggleSaved() {
    const next = !saved; setSaved(next);
    if (accountSaved) { const response = await fetch(`${apiBase()}/public/saved/${encodeURIComponent(slug)}`, { method: next ? 'POST' : 'DELETE', credentials: 'include' }); if (!response.ok) setSaved(!next); return; }
    try { if (next) window.localStorage.setItem(storageKey, '1'); else window.localStorage.removeItem(storageKey); } catch {}
  }
